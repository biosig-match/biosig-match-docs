...
- **実験プロトコルの集約管理**: 実験作成から刺激ファイルのアップロードまでを単一 API で提供し、研究チームがブラウザやモバイルアプリから同一フローで運用できます。CSV とファイルの整合チェックをサーバー側で行うことで、現場でのヒューマンエラーを減らします。
- **セッションライフサイクルの記録**: 計測開始/終了 API を通じてメタデータ・イベントログを保存し、セッションステータス (`link_status`, `event_correction_status`) を後続ジョブに連携します。これにより、データ処理パイプライン全体の起点を統一できます。
- **ジョブキューのハブ**: 刺激登録は Stimulus Asset Processor、セッション終了後の後処理は DataLinker へ送出し、マイクロサービス間の連携を指揮します。各キュー名は環境変数で指定でき、テナントごとの分割にも対応します。
- **データアクセスゲートウェイ**: MinIO から刺激ファイルをストリーミング配信するエンドポイントを提供し、アプリは個別のストレージ資格情報を持つ必要がありません。Auth Manager との連携でロールベースのアクセス制御も担保します。
