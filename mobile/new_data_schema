...
本仕様書は、クライアント（スマホアプリ）からサーバーへ送信されるJSONペイロード内の `payload_base64` フィールドをデコード・解凍した後に得られる、バイナリデータの構造を定義する。
  ...
  * **内部バイナリデータ:**
      ...
      * データは、**単一のヘッダーブロック**と、それに続く**128個のサンプルデータブロック**で構成される連続したバイナリ ストリームである。
...
**[ヘッダーブロック] + ([サンプルデータブロック] x 128回)**

-----

## 3\. ヘッダーブロック仕様

ペイロードの先頭に1つだけ存在する固定長のブロック。このペイロードに含まれるデータの定義情報を持つ。

| フィールド名 | データ型 | サイズ(Bytes) | 説明 |
| :--- | :--- | :--- | :--- |
| `version` | `uint8_t` | 1 | このペイロード構造のバージョン。現行は `0x02`。 |
| `num_channels` | `uint8_t` | 1 | チャンネルの総数。最大24まで。 |
...
| フィールド名 | データ型 | サイズ(Bytes) | 説明 |
| :--- | :--- | :--- | :--- |
| `name` | `char[8]` | 8 | 電極名 (例: "Fp1")。UTF-8エンコード、未使用分は `\0` (ヌル文字)で埋める。 |
...
ヘッダーブロックの直後から、1サンプル分のデータブロックが128回繰り返される。
ブロックの合計サイズは `(num_channels * 3) + 12` Bytes となる。

| フィールド名 | データ型 | サイズ(Bytes) | 説明 |
| :--- | :--- | :--- | :--- |
| `signals` | `uint16_t`配列 | `num_channels` \* 2 | 全チャンネルの信号値 (0-4095)。`electrode_config`の順序に対応する。 |
...
#### `signals` フィールドの特別仕様
  ...
  * 信号値は12bit (0-4095) を想定。
  * `electrode_config` の `type` が `TRIG` **以外**のチャンネルにおいて、`bit 12` (13番目のビット) は汎用的なトリガーフラグとして使用されることがある。
      * `(value & 0x1000) != 0` であれば、そのサンプルで外部トリガーがONになったことを示す。

-----

## 5\. デバイス別データマッピングとOptional項目

Muse 2接続時は、スマホアプリが同じバイナリ構造を維持するため、取得できないデータを「無効値」で埋める。
...
| **サンプルデータ** | | |
| `signals` | 8-24 ch分のデータ | 4 ch分のデータ |
| `accel` | あり | あり |
| `gyro` | あり | あり |
